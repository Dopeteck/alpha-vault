...
const app = {
    ...
    init: function() {
        ...
        this.adController = window.Adsgram?.init({ blockId: "YOUR_BLOCK_ID" });
    },
    ...
    renderJobs: function(jobs) {
        const html = jobs.map(j => `
            <div class="card" onclick="app.tg.openLink('${j.Link || '#'}')">
   ...
   completeCourse: function(courseId, btn) {
    ...
    watchAd: function() {
        if (this.adController) {
            this.adController.show().then(() => {
                this.addPoints(50);
                this.showFloatingReward(50, "Ad Bonus");
            }).catch(err => console.log("Ad skipped or error", err));
        } else {
            // Fallback for testing without AdsGram
            this.addPoints(50);
